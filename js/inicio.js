...
(function ($) {
	...
	$.fn.slider = $.fn.Slider = function (options) {
		...
		var setNavigation = function (el) {
			...
			if (params[el.id].showNavigationButtons) {
				$("<div id='cs-buttons-" + el.id + "' class='cs-buttons'></div>").appendTo($('#slider-' + el.id));
		...
		var init = function (el) {
			...
			}).wrap("<div class='slider' id='slider-" + el.id + "' />");
	...
	// default values
	$.fn.slider.defaults = {
		width: 1520, // width of slider panel
		height: 400, // height of slider panel
		spw: 7, // squares per width
		sph: 15, // squares per height
